...
class Nerve:
    ...
    __init__(simplices):
        ...
        Expects a dict of the form {str(<dim: int>) : {<name of simplex: str>}}, e.g.
        {'0': {'0', '1'}, '1': {'0-1'}}
    extend(simplex):
    ...
    def extend(self, simplex) -> None:
        ...
        while additions:
            ...
            self.simplices[n].add(smplx.name)

    def _pformat(self, dict_data, ind=1):
        ret_str = ""
        for k, v in dict_data.itmes():
            ret_str += "\t" * ind + f"{str(k)}: \n{', '.join([item for item in v])}\n"
        ret_str = "{\n" + ret_str + "}"
        return ret_str

    def __repr__(self) -> str:
        return f"{type(self).__name__}({self._pformat(self.simplices)})"
